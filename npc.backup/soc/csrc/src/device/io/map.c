#include <isa.h>
#include <memory/host.h>
#include <memory/vaddr.h>
#include <device/map.h>

#define IO_SPACE_MAX (2 * 1024 * 1024)

static uint8_t *io_space = NULL;
static uint8_t *p_space = NULL;

uint8_t *new_space(int size) {
    uint8_t *p = p_space;
    // page aligned;
    size = (size + (PAGE_SIZE - 1)) & ~PAGE_MASK;
    p_space += size;
    assert(p_space - io_space < IO_SPACE_MAX);
    return p;
}

static void check_bound(IOMap *map, paddr_t addr) {
    if (NULL == map) {
        Assert(NULL != map, 
            "address (" FMT_PADDR ") is out of bound at pc = " FMT_WORD,
            addr, env_cpu.pc);
    } else {
        Assert(addr <= map->high && addr >= map->low,
            "address (" FMT_PADDR ") is out of bound {%s} [" FMT_PADDR 
            ", " FMT_PADDR "] at pc = " FMT_WORD,
            addr, map->name, map->low, map->high, env_cpu.pc);
    }
}

static void invoke_callback( // 對設備和目標空間的狀態進行更新
    io_callback_t c, paddr_t offset,
    int len, bool is_write
) {
    if (NULL != c) { c(offset, len, is_write); }
}

// 初始化map
void init_map() {
    Log("Init map...");
    io_space = (uint8_t *)malloc(IO_SPACE_MAX);
    assert(io_space);
    p_space = io_space;
}

// 映射的訪問接口，用於將地址addr映射到map所指示的目標空間， 並進行訪問
// 访问时, 可能会触发相应的回调函数, 对设备和目标空间的状态进行更新.
word_t map_read(
    paddr_t addr, int len, IOMap *map
) {
    assert(len >= 1 && len <= 8);
    check_bound(map, addr);
    paddr_t offset = addr - map->low;
    // prepare data to read
    invoke_callback(map->callback, offset, len, false);
    word_t ret = host_read(map->space + offset, len);
    return ret;
}

void map_write(
    paddr_t addr, int len,
    word_t data, IOMap *map
) {
    assert(len >= 1 && len <= 8);
    paddr_t offset = addr - map->low;
    host_write(map->space + offset, len, data);
    invoke_callback(map->callback, offset, len, true);
}
