...
//	csr.io.dmemMMU <> io.memMMU.dmem
	...
	Info("redirect:%x,csr redirect target:%x, alu redirect target:%x, csr valid:%b\n", 
		io.out.bits.decode.cf.redirect.target, csr.io.redirect.target, alu.io.redirect.target, csr.io.redirect.valid)
//	Info(blueFG+"redirect 0x%x"+resetColor+"\n", io.out.bits.decode.cf.redirect.target)
//	Info(blueFG+"alu redirect 0x%x"+resetColor+"\n", alu.io.redirect.target)
//	Info(blueBG+"csr redirect 0x%x"+resetColor+"\n", csr.io.redirect.target)
