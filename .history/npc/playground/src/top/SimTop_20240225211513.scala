...
class SimTopIO extends Bundle {
	...
	if(Settings.get("DiffTestCSR")){val csr=Output(new CSRState)}
	if(Settings.get("DiffTestLoadStore")){val load = Output(new LoadEvent)}
	if(Settings.get("DiffTestLoadStore")){val store = Output(new StoreEvent)}
}
