...
class SimTopIO extends Bundle {
	...
	if(Settings.get("DiffTestCSR")){val csr = Output(CSRState())}
	if(Settings.get("DiffTestLoadStore")){val load = Output(LoadEvent)}
	if(Settings.get("DiffTestLoadStore")){val store = Output(StoreEvent)}
}
