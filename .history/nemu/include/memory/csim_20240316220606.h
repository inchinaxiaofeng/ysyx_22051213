...
#define GET_OFFSET(addr) (paddr_t)(addr&SET_OFFSET_MASK)
#define GET_INDEX(addr) (paddr_t)(&SET_INDEX_MASK >> OLEN)
#define GET_TAG(addr) (paddr_t)(&SET_TAG_MASK >> (OLEN+ILEN))
...
typedef struct tag_line {
	paddr_t tag; // Addr bits wouldn't bigger than paddr.
	bool valid;
} tag_line_t;
...
//bool check_cache_hit(paddr_t set_base, paddr_t addr, uint8_t Level);
