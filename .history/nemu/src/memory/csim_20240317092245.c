...
bool cache_init(
	...
	cache->lv[level].cache_free_num = cache->lv[level].set_num; // Fixme Maybe
...
记录当前Level的Hit&Miss*/
paddr_t check_cache_hit(uint8_t level, paddr_t addr, bool *success) {
	*success = false;
...
paddr_t get_cache_free_line(uint8_t level, paddr_t addr, bool *success) {
	*success = false;
