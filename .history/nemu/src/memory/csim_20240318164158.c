...
void do_cache_update_line(
	...
	paddr_t old_mapping_addr = (cache->lv[level].[index][way].tag << (olen+ilen)) | (index << olen) | 0;
	paddr_t target_mapping_addr = (new_tag << (olen+ilen)) | (index << olen) | 0;
		...
		*tmp_val = pmem_read(target_mapping_addr, cls);
		...
		if (isWriteBack) {
			...
			for (i = 0; i < trans_count; i++) {
				isInsideLine = byteArr2word_t(line + i*sizeof(word_t), sizeof(word_t), tmp_val); if (likely(in_pmem(old_mapping_addr + i*sizeof(word_t))))
		...
		case OPERATION_READ:
			...
			for (i = 0; i < get_line_count; i++) {
				...
				if (!(*hit_l1)) { // Miss
					...
					do_cache_update_line(0, index, hit_way_l1, )
				}
