...
返回值
	返回一个转换折扣，其等与【源字节-目标字节】，即sizeof(word_t) - byte_len
行为
	...
	半信息转换时，将信息全部写入byte_len格式，并将信息源高位信息丢弃
*/
/*
三种情况：cls < word_t、cls > word_t、cls = word_t
如果处于情况一，我需要返回一个信息，并将word_t中的低位信息写入cls
如果处于情况二，我需要将全部的word_t写入cls，并保持cls高位不变，返回信息
如果处于情况三，k
*/
