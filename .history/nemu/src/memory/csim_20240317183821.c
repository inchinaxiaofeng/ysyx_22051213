...
void do_cache_update_line(
	...
	if(cls <= MUXDEF(CONFIG_ISA64, 8, 4)) { // 这种情况不需要考虑多次写入/读出以保证所有数据都被正确的写入或读出
		...
		if(0 == new_offset)
			do_cache_write_line(level, new_offset, index, way, paddr_read(target_mapping_addr, cls), cls);
		else {
			do_cache_write_line(level, new_offset, index, way, paddr_read(target_mapping_addr, cls), cls-new_offset);
		}
	} else {
