...
--------------------------------------------------------------------
值得注意的是，在连续传输的情况下，除了CPU给出的地址之外的任何地址，
都可能发生访问的pmem地址是内存映射地址或非法地址。
所以需要在执行访存函数时，通过函数in_pmem函数避免访问这些地址。
在单次访问的情况下，在调用cacheupdate之前，
访问的地址已经被in_pmem函数检查过一次了，所以不会发生访问非法地址空间的情况。
--------------------------------------------------------------------
		...
		*tmp_val = pmem_read(target_mapping_addr, cls);
		...
		if (isWriteBack) {
			...
			for (i = 0; i < trans_count; i++) {
				...
				if (likely(im_pmem(old_mapping_addr + i*sizeof(word_t))))
