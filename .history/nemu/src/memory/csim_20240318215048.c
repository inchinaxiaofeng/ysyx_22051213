...
void do_cache_update_line(
	...
	switch (cache->lv[level].line[index][way].swap_policy)
	{
		...
		*/
		...
		case OPERATION_WRITE:
			...
			for (i = 0; i < get_line_count; i++) {
				...
				assert(word_t2byteArr(line, 1==get_line_count?byte_len:0==i?cls-offset:cls, tmp_val));
			...
			if (0 != last_get_line_byte_len) {
				...
				assert(word_t2byteArr(line, last_get_line_byte_len, tmp_val));
