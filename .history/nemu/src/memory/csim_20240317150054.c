...
参数
	...
	read_len 读取的长度
*/
word_t do_cache_read(uint8_t level, paddr_t addr, uint128_t byte_mask) {
	bool *hit;
	paddr_t hit_way;
}
...
参数
	...
	byte_mask 对访问的数据的范围进行遮罩, 1 bit masks 1 byte
返回值
	返回值依据需要读取的数据大小
	如果oper_style为read的话，则返回读出的数据
	如果oper_style为write的话，则返回值无意义
*/
word_t do_cache_op(paddr_t addr, char oper_style, uint128_t byte_mask) {
