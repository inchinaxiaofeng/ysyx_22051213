...
参数
	...
	* is_valid 作为返回值使用，用于指定是否成功读取到数据
返回值
	当基于offset的读取范围超出当前line的范围，返回false，表示cache read out of line
	当基于offset的读取范围没有超出当前line的范围，返回true
	当读取的值的valid标签为false，则返回true，但它不具有任何意义
行为
	当越界读取时，仅读取从offset开始的line内的合法空间，并设置返回值
	当cacheline的valid为false时，不对* read_date做任何操作并返回，返回值为true，但是它已经不具有任何意义
*/
bool do_cache_read_line(
	uint8_t level, paddr_t offset, paddr_t index,
	paddr_t way, uint8_t *read_data, int byte_len, bool *is_valid
) {
	if (cache->lv[level].line[index][way].valid) return true;
...
*/
	...
	if (trans_count == 0) {
		...
		*tmp_val = pmem_read(new_mapping_addr, cls);
...
word_t do_cache_op(paddr_t addr, char oper_style, int byte_len) {
	...
	bool *hit_l1_wb, *hit_l2_wb, *hit_l3_wb;
