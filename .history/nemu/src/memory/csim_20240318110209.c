...
void do_cache_update_line(
	...
	//感觉很容易发生内存泄露
	for (size_t len_offset = 0;
		!byteArr2word_t(line+len_offset*sizeof(word_t), cls-len_offset*sizeof(word_t), tmp_val);
		paddr_write(old_mapping_addr, cls-len_offset*sizeof(word_t), tmp_val), len_offset++
	);
	for (size_t i = 0; i < count; i++)
	{
		if (byteArr2word_t(line+len_offset*sizeof(word_t), cls-len_offset*sizeof(word_t), tmp_val) break;
)
	}
