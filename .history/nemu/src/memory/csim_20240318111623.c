...
void do_cache_update_line(
	...
	int trans_count = 0;
	paddr_t last_trans_offset = 0;

	// 将Cache中的数据写入
	assert(do_cache_read_line(level, 0, index, way, line, cls));
	trans_count = cls / sizeof(word_t);
	last_trans_offset = cls % sizeof(word_t);
	for (size_t i = 0; i <= trans_count; i++) {
		byteArr2word_t(line+i*sizeof(word_t), cls-i*sizeof(word_t), tmp_val);
		paddr_write(old_mapping_addr, 0==trans_count?last_trans_offset:cls-i*sizeof(word_t), tmp_val);
	}
	
		if (byteArr2word_t(line+i*sizeof(word_t), cls-i*sizeof(word_t), tmp_val)) break;
		paddr_write(old_mapping_addr, cls-i*sizeof(word_t), tmp_val);
	}
