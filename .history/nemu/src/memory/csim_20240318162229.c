...
*/
		...
		paddr_t last_get_line_byte_len = 1==get_line_count ? byte_len : byte_len+offset-cls*get_line_count;
		...
		case OPERATION_READ:
			...
			for (i = 0; i < get_line_count; i++) {
				hit_way_l1 = check_cache_hit(1, addr+i*cls, hit_l1);
				if (*hit_l1) { // Hit
					assert(do_cache_read_line(0, 0==i?offset:0, index+i, hit_way_l1, line, 1==get_line_count?byte_len:0==i?cls-offset:cls));
				} else { // Miss
