...
static bool cache_lv_set_val(
	...
	cache->lv[level].set_offset_mask = ()pow(2, cache->lv[level].olen)-1;
	cache->lv[level].set_index_mask = (pow(2, cache->lv[level].ilen)-1) << cache->lv[level].olen;
	cache->lv[level].set_tag_mask = (pow(2, cache->lv[level].tlen)-1) << (cache->lv[level].olen + cache->lv[level].ilen);
}
