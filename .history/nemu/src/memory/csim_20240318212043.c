...
当cls的大小足够单次访存完成传输，表现为进行一次last访存或者一次full访问
当cls的大小不足以单次访存完成传输，k
	...
	当trans_count == 0时，last_trans_offset就是cls，可以视作连续传输了0次后，做last传输
	trans_count >= 1，将进入for循环尝试进行连续传输
--------------------------------------------------------------------
值得注意的是，在连续传输的长度大于1的情况下，除了传输起始地址，都可能发生访问的pmem地址是内存映射地址或非法地址。
