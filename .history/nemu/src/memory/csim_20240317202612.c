...
inline void word_t2byteArr(
	uint8_t *byte, word_t word
) {
...
inline word_t byteArr2word_t(
	uint8_t *byte, int byte_len
) {
	word_t ret_val = 0;
...
*/
void do_cache_read_line(
	uint8_t level, paddr_t offset, paddr_t index,
	paddr_t way, uint8_t *byteArr, int byte_len
) {
	uint32_t cls = cache->lv[level].cache_line_size;
	Assert(cls-offset <= byte_len, "Read out of line.");
	for (size_t i = 0; i < byte_len; i++)
		byteArr[i] = cache->lv[level].line[index][way].data[offset+i] << i*8;
	return;
}

/*
描述
	对某一行cacheline进行写操作，单次可写入word_t宽度的数据。
	写入超位宽数据应当多次调用函数。
参数
	level 用于获取cache级
	offset addr的offset字段
...
void do_cache_write_line(
	...
	for (size_t i = 0; i < byte_len; i++)
		cache->lv[level].line[index][way].data[offset+i] = write_data[i];// (wrt_val&UINT8_MAX) >> i*8;
	return;
}
...
void do_cache_update_line(
	...
	if(cls <= MUXDEF(CONFIG_ISA64, 8, 4)) { // 这种情况不需要考虑多次写入/读出以保证所有数据都被正确的写入或读出
		paddr_write(old_mapping_addr, cls, do_cache_read_line(level, old_offset, index, way, cls));
		word_t tmp_val = paddr_read(target_mapping_addr, cls);
		
		do_cache_write_line(level, new_offset, index, way,, cls);
	} else {
		// TODO 优先考虑跑通
	}
