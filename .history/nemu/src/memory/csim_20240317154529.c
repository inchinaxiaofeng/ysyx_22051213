...
/*
描述
	Cache进行读访问。对Cache进行一般性操作，不对Level做区别。
参数
	addr 访问地址
	byte_len 对访问的数据的范围进行选定
*/
word_t do_cache_read(uint8_t level, paddr_t offset, paddr_t index, paddr_t way, int byte_len) {
	// 分别获得当前byte_len需要访问几个line和在第一个line与最后一个line的offset
	int line_num = byte_len / cache->lv[level].cache_line_size;
	int last_offset = addr
	word_t ret_val = 0;
	for (size_t i = 0; i < byte_len; i++) {
		ret_val = cache->lv[level].line[index][way].data[offset+i];
	}
	ret_val = cache->lv[level].line[index][way].data
}
