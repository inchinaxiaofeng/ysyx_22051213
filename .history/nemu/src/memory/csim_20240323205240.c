...
word_t do_cache_op(paddr_t addr, char oper_style, int byte_len, word_t write_data) {
	...
	case 1: // Only have L1 Cache
		...
		part_access_len
			部分访问cacheline的长度
			当full_access为0时，part_access_len需要加上offset
			当full_access大于0时，part_access_len不需要加offset（offset为0）
line
|     |     |     |     |     |     |     |
|   data    |
   |  data  |
		...
		*/
		...
		paddr_t part_access_len = 0==full_access_count ? offset+byte_len : byte_len;
