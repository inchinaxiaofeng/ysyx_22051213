	...
	for (size_t l = 0; l < MAX_LEVEL; l++) {
		for (size_t s = 0; s < SET_NUM; s++) {
			for (size_t w = 0; w < WAY_NUM; w++) {
				cache->lv[l].tagArr[s][w].valid = false;
			}
		}
	}
...
paddr_t check_cache_hit(paddr_t addr, uint8_t level) {
	...
	for (size_t w = 0; w < WAY_NUM; w++) {
		if (tage == cache->lv[level].tagArr[index][w].tag &&
			true == cache->lv[level].tagArr[index][w].valid) {
...
/* 获取当前set中可用的way，如果没有，则要找到被替换的块 */
int get_cache_free_line(paddr_t addr, int level) {
	int free_index;
}

/* 将数据写入cache line中 */
void set_cache_line(paddr_t addr, int level) {
