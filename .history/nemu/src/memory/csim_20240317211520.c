...
行为
	...
	对于byteArr，不改变多余高位
	当word_t类型不能被写入byteArr时，不做写入操作并设置返回值
*/
inline bool word_t2byteArr(
	uint8_t *byte, int byte_len, word_t word
) {
	if (byte_len <= sizeof(word_t)) return false;
...
参数
	byte uint8_t * 类型数据
	byte_len 共将byteArr中的多少个数据
行为
	将byteArr的数据写入word_t中，无论是否能够写入都会尝试。
*/
...
void do_cache_read_line(
	uint8_t level, paddr_t index,
	paddr_t way, uint8_t *read_data
) {
	uint32_t cls = cache->lv[level].cache_line_size;
	for (size_t i = 0; i < cls; i++)
		read_data[i] = cache->lv[level].line[index][way].data[i];
	return;
}
...
void do_cache_write_line(
	...
	paddr_t way, uint8_t* write_data
) {
	uint32_t cls = cache->lv[level].cache_line_size;
...
void do_cache_update_line(
	...
	if(cls <= MUXDEF(CONFIG_ISA64, 8, 4)) { // 这种情况不需要考虑多次写入/读出以保证所有数据都被正确的写入或读出
		// 先存入数据
		do_cache_read_line(level, old_offset, index, way, line);
		...
		do_cache_write_line(level, new_offset, index, way, line);
	} else {
		// TODO 优先考虑跑通
	}
