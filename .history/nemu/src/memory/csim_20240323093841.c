...
/*
描述
	将word_t格式转化为byteArr格式
参数
	byte		目标格式，作为返回值处理
	byte_len	期望转化的字节长度，不应当超过byte的最大长度
	word		源格式，不传入指针类型以避免错误的操作
返回值
	返回一个转换折扣，其等与【源字节-目标字节】，即 sizeof(word_t) - byte_len
行为
	将sizeof(word_t) - byte_len称为转换折扣
	当转换折扣 <= 0时，为全信息转换，此时格式转换前后，信息不会损失
	当转换折扣 > 0时，为半信息转换，此时格式转化前后，信息由所损失
...
参数
	...
	word		目标格式, 作为返回值处理
返回值
	返回一个转换折扣，其等与【源字节-目标字节】，即 byte_len - sizeof(word_t)
行为
	将sizeof(word_t) - byte_len称为转换折扣
	当转换折扣 <= 0时，为全信息转换，此时格式转换前后，信息不会损失
	当转换折扣 > 0时，为半信息转换，此时格式转化前后，信息由所损失
...
返回值
	返回访问余量，即 cls - offset - byte_len
行为
	Define the cache_line_size(cls) len of the line after
	setting the offset as the line_margin
	将【设置offset后line的剩余长度】成为【行余量】
	将【行余量 - 访问长度】成为【访问余量】
	访问余量 >= 0 时，称为行内访问
	访问余量 < 0 时，称为越行访问
	行内访问则正确访问数据，越行访问则仅访问【行余量】部分
注意
	在调用这个函数之前，应当确保其line的valid是true。
*/
static inline int do_cache_read_line(
	uint8_t level, paddr_t offset, paddr_t index, paddr_t way,
	uint8_t *read_data, int byte_len
) {
	uint32_t cls = cache->lv[level].cache_line_size;
	int line_margin = cls - offset;
	int access_margin = line_margin - byte_len;
	for (size_t i = 0; i < (access_margin>=0 ? byte_len : line_margin); i++) {
...
返回值
	返回访问余量，即 cls - offset - byte_len
	当基于offset的写入范围没有超过line的范围，即offset+cls < word_t，则返回一个负数
	当基于offset的写入范围与line的现有大小相同，即
	当基于offset的写入范围超过当前line范围，返回false，表示cache write out of line
	当基于offset的写入范围没有超出当前line范围，返回true
行为
	将【设置offset后line的剩余长度 - 访问长度】成为【访问余量】
	访问余量 >= 0 时，称为行内访问
	访问余量 < 0 时，称为越行访问
	行内访问则正确访问数据，越行访问则仅访问【余量】部分

	当越界写入时，仅写入从offset开始的line内的合法空间，并设置返回值
*/
int do_cache_write_line(
	uint8_t level, paddr_t offset, paddr_t index, paddr_t way,
	uint8_t* write_data, int byte_len
) {
	uint32_t cls = cache->lv[level].cache_line_size;
