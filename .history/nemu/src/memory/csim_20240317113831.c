...
void do_cache_op(paddr_t addr, char oper_style) {
	...
	case 1: hit_way_l1 = check_cache_hit(1, addr, hit_l1);
		cache->lv[1].hit_count++;
		if (true == *hit_l1) break;	
	case 2: hit_way_l2 = check_cache_hit(2, addr, hit_l2);
		cache->lv[2].hit_count++;
		if (true == *hit_l2) break;
	case 3: hit_way_l3 = check_cache_hit(3, addr, hit_l3);
		cache->lv[3].hit_count++;
		if (true == *hit_l3) break;
	default: // TODO MISS
		break;
	}
