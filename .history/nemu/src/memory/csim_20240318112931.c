...
void do_cache_update_line(
	...
	/*
	trans_count == 0
		表示cls比word_t小，访存时应当以cls为访问长度
		读写主存都在一次完成。
	trans_count == 1
		表示cls与word_t有相同大小，访存时既可以以cls为访问长度，也可以以sizeof(word_t)为访问长度
		读写主存都在一次完成
	trans_count > 1
		表示cls比word_t大，采用连续传输的方式与主存交互。
		连续传输的访问长度为sizeof(word_t)。
		如果last_trans_offset为0，则以sizeof(word_t)为访问长度, 访问trans_count+1次
		读写主存需要
	*/
	int trans_count = cls / sizeof(word_t); // trans_count = 0 表示一次访存就可以完成
	...
	for (size_t i = 0; i <= trans_count; i++) {
		byteArr2word_t(line+i*sizeof(word_t), cls-i*sizeof(word_t), tmp_val);
		paddr_write(old_mapping_addr, 0==trans_count?last_trans_offset:sizeof(word_t), tmp_val);
	}
	
	// 将内存中的数据写入Cache
	for (size_t i = 0; i <= trans_count; i++) {
		tmp_val = paddr_read(target_mapping_addr, 0==trans_count?cls);
	}
