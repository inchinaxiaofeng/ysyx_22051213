...
void do_cache_update_line(
	...
	paddr_t index = (old_mapping_addr&cache->lv[level].set_index_mask) >> cache->lv[level].olen;
	uint32_t olen = cache->lv[level].olen;
	uint32_t cls = cache->lv[level].cache_line_size;
	Assert(index == ((target_addr&set_index_mask) >> olen),
		"Bad cache mapping!");
	...
	word_t tmp_val = 0;
	if(cls <= MUXDEF(CONFIG_ISA64, 8, 4)) {
		paddr_write()
		tmp_val = paddr_read(target_addr, cls);
	} else {
