...
*/
	...
	if (isWriteBack) {
		...
		for (i = 0; byteArr2word_t(line + i*sizeof(word_t), cls - i*sizeof(word_t), tmp_val); i++){}
		while (byteArr2word_t(line))
		{
			/* code */
		}
		
		for (i = 0; i < full_trans_count; i++) {
			trans_discount = byteArr2word_t(line + i*sizeof(word_t), sizeof(word_t), tmp_val);
