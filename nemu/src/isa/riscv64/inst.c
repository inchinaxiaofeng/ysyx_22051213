...
enum {
  TYPE_R, TYPE_I, TYPE_U, TYPE_S, TYPE_J, TYPE_B,TYPE_C,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT(        BITS(src1, 31, 0) << BITS(s->isa.inst.val, 24, 20), 32));
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(        BITS(src1, 31, 0) >> BITS(s->isa.inst.val, 24, 20), 32));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((signed)BITS(src1, 31, 0) >> BITS(s->isa.inst.val, 24, 20), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (signed)src1 >>         BITS(src2, 5, 0));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT((signed)BITS(src1, 31, 0) >> BITS(src2, 4, 0), 32));

  // Unconditional Jumps
  INSTPAT("? ?????????? ? ???????? ????? 11011 11", jal    , J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = (src1 + imm)&((uint64_t)(-1) << 2));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = SEXT(BITS((signed)src1 % (signed)src2, 31, 0), 32));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , C, R(rd) = csr(imm), csr(imm) = src1 & csr(imm));

  // machine-mode Exception Return
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = isa_mret()+4, difftest_skip_ref(); IFDEF(CONFIG_CSR_INSTR_TRACE, printf("\33[1;46mmret--dnpc:\33[0m0x%016lx\n", s->dnpc)));

  // Environment Call and Breakpoints
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0x0b, s->pc); IFDEF(CONFIG_CSR_INSTR_TRACE, printf("\33[1;46mecall--dnpc:\33[0m0x%016lx\n", s->dnpc)));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0

  // 
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, INV(s->pc)); // 必须在最后！
