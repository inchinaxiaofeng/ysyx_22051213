...
/*
difftest的头文件声明
  定义了一些diffteset需要用到了函数
    其中包括：
      difftest_skip_ref用于跳过一些指令（real）
      difftest_skip_dut用于跳过一些指令（自己CPU）
      difftest_set_patch
      difftest_step单步指令执行
*/
...
void difftest_skip_ref(); // 跳过对照指令
void difftest_skip_dut(int nr_ref, int nr_dut); // 跳过测试指令
void difftest_set_patch(void (*fn)(void *arg), void *arg); // 
void difftest_step(vaddr_t pc, vaddr_t npc); // 单步指令执行
